...
public class PayConsultaionWebController extends BaseController {
    @Autowired
    private IPayConsultaionService payConsultaionService;
    private Logger logger = LoggerFactory.getLogger(PayConsultaionWebController.class);
    ...
    public Result del(@PathVariable String  id) {
     ...
     /**
       ...
       */
       ...
       @PostMapping("/add")
       public Result add(@RequestBody PayConsultaion  payConsultaion ) {
           boolean  boo = payConsultaionService.save(payConsultaion);
           return Results.successWithData(boo, BaseEnums.SUCCESS.desc());
       }
     /**
       	 * 更新
       ...
       public Result edit(@RequestBody PayConsultaion   payConsultaion ,UserVO userVO){
        payConsultaion .setUpdateDate(new Date());
        payConsultaion .setUpdateBy(userVO.getId());
        payConsultaion .setUpdater(userVO.getUserName());
        boolean boo = payConsultaionService.updateById( payConsultaion);
        return  Results.successWithData(boo,BaseEnums.SUCCESS.desc());
    }
