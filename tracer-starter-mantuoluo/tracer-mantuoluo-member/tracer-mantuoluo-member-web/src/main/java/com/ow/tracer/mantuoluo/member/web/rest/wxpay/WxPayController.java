package com.ow.tracer.mantuoluo.member.web.rest.wxpay;
...
import com.ow.tracer.mantuoluo.member.pay.entity.H5ScencInfo;
...
public class WxPayController extends WxPayApiController {
	...
	@Autowired
	private IOrderTutorService iOrderTutorService;
	@Autowired
	...
	public String pay_notify(HttpServletRequest request) {
		...
		if (PaymentKit.verifyNotify(params, WxPayApiConfigKit.getWxPayApiConfig().getPaternerKey())) {
			if (("SUCCESS").equals(result_code)) {
				PayConsultaion payConsultaion = iPayConsultaionService.getById(out_trade_no);
				MemberUser memberUser = memberUserService.getById(payConsultaion.getMemberId());

						payConsultaion.setReaded(1);
				payConsultaion.setStatus(1);
				payConsultaion.setPrepayId(transaction_id);
				iPayConsultaionService.updateById(payConsultaion);
				if(payConsultaion.getType()==0){
					AdminNotice adminNotice = new AdminNotice();
					adminNotice.setContent("用户："+memberUser.getOpenId()+",支付了计算订单，金额："+payConsultaion.getTotalFee()+"元");
					adminNotice.setType(0);
					adminNotice.setCreateTime(DateTime.now().toTimestamp());
					iAdminNoticeService.save(adminNotice);
				}else{
					ConsultationInfo consultation = iConsultationInfoService.getById(payConsultaion.getConsultationId());
					AdminNotice adminNotice = new AdminNotice();
					adminNotice.setContent("用户："+memberUser.getOpenId()+",支付了咨询订单，金额："+payConsultaion.getTotalFee()+"元");
					adminNotice.setType(1);
					adminNotice.setCreateTime(DateTime.now().toTimestamp());
					iAdminNoticeService.save(adminNotice);
					OrderTutor orderTutor = new OrderTutor();
					orderTutor.setConId(consultation.getId());
					orderTutor.setMemberId(consultation.getMemberId());
					orderTutor.setTutorId(consultation.getTutorId());
					orderTutor.setPayId(payConsultaion.getId());
					orderTutor.setTofee(payConsultaion.getTotalFee()*siteSetting.getTwoPrice());
					orderTutor.setType(1);//选择导师费用
					iOrderTutorService.save(orderTutor);
					Consultation zxConsultation = consultationService.getById(consultation.getConId());
					PayConsultaion zxpayConsultaion = iPayConsultaionService.getOne(new QueryWrapper<PayConsultaion>().eq("consultation_id",zxConsultation.getId()));
					OrderTutor zxOrderTutor = new OrderTutor();
					zxOrderTutor.setConId(zxConsultation.getId());
					zxOrderTutor.setMemberId(zxConsultation.getMemberId());
					zxOrderTutor.setTutorId(zxConsultation.getTutorId());
					zxOrderTutor.setPayId(zxpayConsultaion.getId());
					zxOrderTutor.setTofee(zxpayConsultaion.getTotalFee()*siteSetting.getTwoPrice());
					zxOrderTutor.setType(0);//选择导师费用
					iOrderTutorService.save(orderTutor);
				}
				// 更新订单信息
